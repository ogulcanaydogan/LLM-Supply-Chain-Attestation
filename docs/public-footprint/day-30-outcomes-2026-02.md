...
| Upstream PRs merged | >=1 | 2 | achieved | `docs/public-footprint/external-contribution-log.md` |
...
| CI pass-rate (rolling 30-day) | >=95% | 95.31% (61/64) | achieved | `.llmsa/public-footprint/20260306T150113Z/ci-health.json` |
| CI pass-rate (post-hardening baseline) | >=95% | 95.31% (61/64) | achieved | `.llmsa/public-footprint/20260306T150113Z/ci-health.json` |
...
3. Upstream cadence process became explicit and traceable (12h follow-up workflow while lanes were open), and maintainer-requested revisions were pushed to `cosign#4710`.
...
1. OPA docs contribution lane remained closed-unmerged (`#8343`, `#8346`) based on maintainer scoping guidance.
2. Two backup lanes were closed as superseded after second merge conversion (`scorecard#4960`, `slsa-github-generator#4468`).
...
1. Maintain CI/workflow stability above strict threshold and rerun consistency checks on each evidence refresh.
2. Keep evidence pack + roadmap outputs frozen to a single artifact pair and regenerate only on material status changes.
3. Keep follow-up automation non-posting after closure to avoid stale external nudges.
