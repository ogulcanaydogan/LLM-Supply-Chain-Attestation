...
| WS2: CI pass-rate | rolling `>=95%` | rolling `95.4% (83/87)`; post-hardening `98.77% (80/81)` | complete |
...
1. **Strict threshold met.** Rolling CI pass rate is `95.4%` (`83/87`), above the `>=95%` requirement.
2. **Post-hardening trend remains strong.** Since `2026-02-19T16:08:22Z`, post-hardening pass rate is `98.77%` (`80/81`).
...
2. **WS2 (CI pass-rate):** met — strict rolling threshold satisfied (`95.4%`).
